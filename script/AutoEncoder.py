...
class AutoEncoder(torch.nn.Module):
    def __init__(self, in_Size=128, out_Size=64, inter_Size=96,
                 em_len=64, radical=32, angular=16):
        super(AutoEncoder, self).__init__()
        self.in_size = in_Size
        self.out_size = out_Size
        self.inter_size = inter_Size
        self.em = nn.Embedding(100, em_len, padding_idx=0)
        self.sf = spk.representation.SymmetryFunctions(n_radial=radical, n_angular=angular, zetas=[1],
                                                       elements=frozenset((28,)), initz='onehot',pairwise_elements=True)
