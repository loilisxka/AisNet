...
class Trainer:
    ...
    def train(self, device, n_epochs=sys.maxsize, em_net=None, pbc=False):
        ...
        Args:
            ...
            em_net (nn.Module):the Embedding Net to SchNet
        ...
        try:
            for _ in range(n_epochs):
                ...
                for train_batch in train_iter:
                    ...
                    # 在字典张量中插入new_Embedding模块计算结果
                    if pbc is False:
                        if em_net is not None:
                            train_batch = EmbeddingNet.add_data(train_batch)
                            train_batch[Properties.new_embedding].to(device)
                            B, A = train_batch[Properties.new_embedding].shape[0], train_batch[Properties.new_embedding].shape[1]
                            train_batch[Properties.new_embedding] = EmbeddingNet(train_batch[Properties.new_embedding]).reshape(B, A, -1)
                            # print(train_batch[Properties.new_embedding].shape)
                    else:
                        if em_net is not None:
                            train_batch = EmbeddingNet.add_data(train_batch)
                            train_batch[Properties.new_embedding].to(device)
                            B = train_batch[Properties.new_embedding].shape[0]
                            A = train_batch[Properties.new_embedding].shape[1]
                            train_batch[Properties.new_embedding], other = EmbeddingNet(
                                train_batch[Properties.new_embedding].view(B * A, -1))
                            train_batch[Properties.new_embedding] = train_batch[Properties.new_embedding].reshape(B, A, -1)
                ...
                if self.epoch % self.validation_interval == 0 or self._stop:
                    ...
                    for val_batch in self.validation_loader:
                        ...
                        if pbc is False:
                            if em_net is not None:
                                val_batch = EmbeddingNet.add_data(val_batch)
                                val_batch[Properties.new_embedding].to(device)
                                B, A = val_batch[Properties.new_embedding].shape[0], \
                                       val_batch[Properties.new_embedding].shape[1]
                                val_batch[Properties.new_embedding] = EmbeddingNet(
                                    val_batch[Properties.new_embedding]).reshape(B, A, -1)
                                # print(val_batch[Properties.new_embedding].shape)
                        else:
                            if em_net is not None:
                                val_batch = EmbeddingNet.add_data(val_batch)
                                val_batch[Properties.new_embedding].to(device)
                                B = val_batch[Properties.new_embedding].shape[0]
                                A = val_batch[Properties.new_embedding].shape[1]
                                val_batch[Properties.new_embedding], other = EmbeddingNet(
                                    val_batch[Properties.new_embedding].view(B * A, -1))
                                val_batch[Properties.new_embedding] = val_batch[Properties.new_embedding].reshape(B, A, -1)
