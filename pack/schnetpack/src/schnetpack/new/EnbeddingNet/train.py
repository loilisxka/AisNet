...
new_dataset = AtomsData("../mydata/fe/fe.db", collect_triples=True)
...
train_loader = spk.AtomsLoader(train, batch_size=batch, shuffle=True)
test_loader = spk.AtomsLoader(test, batch_size=batch, shuffle=True)
...
for i in range(epoch):
    ...
    print("测试集Loss为：{}".format(total_test_loss/test_len))
    if best_test_loss >= total_test_loss:
        best_test_loss = total_test_loss
        torch.save(model, "Fe_AutoEncoder.pth")
