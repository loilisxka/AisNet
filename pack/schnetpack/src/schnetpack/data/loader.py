...
def _collate_aseatoms(examples):
    ...
    pos_ = batch[Properties.position]
    batch_size = pos_.size()[0]
    batch[Properties.pbc_offset] = torch.zeros([batch_size, 96, 95, 3])
    pos = torch.clone(pos_)
    for i in range(batch_size):
        pos_ = pos[i].squeeze(0).tolist()
        # cell不一样
        cell = batch[Properties.cell][i].squeeze(0).tolist()
        atoms = Atoms(positions=pos_, cell=cell, pbc=True)
        nh1, nh2, s = neighbor_list("ijS", atoms, 5.0, self_interaction=False)
        for j in range(len(nh1)):
            batch[Properties.pbc_offset][i, nh1[j], nh2[j] - 1, :] = torch.tensor(s[j])
